...
def test_the_age_helper_is_the_shared_one() -> None:
    ...
    Phase 2 finished the job: every age on this tab is days, and both the
    number and the word come from app.js."""
    src = _strip_comments(CREATORS)
    for helper in ("fmtAgeDays", "agePhrase"):
        assert re.search(
            rf"import \{{[^}}]*\b{helper}\b[^}}]*\}} from \"/js/app\.js\"", src), (
            f"the shared {helper} is not imported")
    assert "mo ago" not in src
    assert "export function fmtAgeDays" in APP, "the shared helper must exist"
    # THE BOUNDARY, in one sentence: `ageDays` is the NUMBER, because the board
    # sorts on it; `ageWord` and `agoWord` are the shared words, because the
    # page prints them. The "4d" this file used to print was a fifth spelling
    # of an age across the app.
    assert "function ageDays(iso)" in src, "the sort key needs the number"
    assert "function daysWord(" not in src, (
        "the local spelling of an age is gone; app.js owns the words")
    assert src.count("ageWord(") + src.count("agoWord(") >= 6, (
        "a publication date is still formatted by hand")
...
def test_one_type_scale_and_no_raw_font_size_in_the_swept_stylesheets() -> None:
    ...
    for name in sorted(p.name for p in WEB.glob("*.css")):
...
def test_one_sortable_header_and_the_views_call_it() -> None:
    ...
    for view in ("xpoints", "pipelines", "template", "creators"):
