...
@mcp.tool()
def make_chart(spec_json: str) -> str:
    """Render a chart server-side and get back a marker to embed in your answer.

    Send a JSON spec; a PNG is written where the chat pane can serve it, and
    this returns a line ``CHART_SAVED chart_id=<id>``. To show the chart, put
    the marker ``[chart:<id>]`` on its own line in your prose exactly where the
    chart belongs — the pane replaces the marker with the image. An answer that
    never embeds the marker shows no chart.

    Spec:
        {"type": "bar"|"barh"|"line"|"scatter", "title": str,
         "x": [...], "y": [...],                       # single series, OR
         "series": [{"name": str, "x": [...], "y": [...]}, ...],
         "xlabel": str, "ylabel": str, "annotate": bool}

    Design rules (enforced here — do not fight them): one hue (#2a78d6) when
    the chart shows magnitude of one measure; additional series get the
    engine's reference palette, never a rainbow. ``annotate: true`` prints the
    value on each mark — use it for <=15 marks. A legend appears only when
    there is more than one series. Sort bars in SQL before charting; keep to
    <=25 categories or aggregate.

    Args:
        spec_json: The spec as a JSON string.

    Returns:
        "CHART_SAVED chart_id=<id>" plus a one-line caption, or the parse /
    ...
    try:
        ...
        return f"spec_json is not valid JSON: {exc}\nRemediation: send the spec object itself, JSON-encoded."
    kind = spec.get("type")
    if kind not in ("bar", "barh", "line", "scatter"):
        return f"type must be bar|barh|line|scatter, got {kind!r}."
    if "series" in spec and spec["series"]:
        series = spec["series"]
    elif "x" in spec and "y" in spec:
        series = [{"name": None, "x": spec["x"], "y": spec["y"]}]
    else:
        return "spec needs either x and y, or a non-empty series list."
    for i, s in enumerate(series):
        xs, ys = s.get("x"), s.get("y")
        if not isinstance(xs, list) or not isinstance(ys, list) or len(xs) != len(ys) or not xs:
            return (
                f"series[{i}] needs non-empty x and y lists of equal length "
                f"(got {len(xs) if isinstance(xs, list) else '?'} x, "
                f"{len(ys) if isinstance(ys, list) else '?'} y)."
            )
    ...
    if kind == "barh":
        figsize = (7.0, max(0.42 * n_max + 1.3, 2.2))
    else:
        figsize = (7.0, 4.4)
    fig, ax = plt.subplots(figsize=figsize, dpi=150)
    fig.patch.set_facecolor(_BG)
    ax.set_facecolor(_BG)
    annotate = bool(spec.get("annotate"))
    multi = len(series) > 1

    for i, s in enumerate(series):
        color = _SERIES_COLORS[i % len(_SERIES_COLORS)]
        xs, ys = s["x"], [float(v) for v in s["y"]]
        label = s.get("name")
        if kind == "bar":
            ax.bar(xs, ys, color=color, zorder=3, label=label,
            ...
            ax.barh(xs, ys, color=color, height=0.62, zorder=3, label=label)
            if annotate:
                span = max(ys) - min(min(ys), 0) or 1.0
                for x, y in zip(xs, ys):
                    ax.text(y + span * 0.012, x, f"{y:.2f}", va="center",
                            fontsize=8, color=_INK2)
        elif kind == "line":
            ...
            if annotate:
                for x, y in zip(xs, ys):
                    ax.annotate(f"{y:.2f}", (x, y), textcoords="offset points",
                                xytext=(0, 6), ha="center", fontsize=8, color=_INK2)
        else:  # scatter
            ax.scatter(xs, ys, color=color, s=26, zorder=3, label=label)
            if annotate:
                for x, y in zip(xs, ys):
                    ax.annotate(f"{y:.2f}", (x, y), textcoords="offset points",
                                xytext=(0, 6), ha="center", fontsize=8, color=_INK2)
    ...
    if title:
        ax.set_title(title, fontsize=11.5, weight="bold", color=_INK, loc="left")
    if spec.get("xlabel"):
        ax.set_xlabel(str(spec["xlabel"]), fontsize=8.5, color=_INK2)
    if spec.get("ylabel"):
        ax.set_ylabel(str(spec["ylabel"]), fontsize=8.5, color=_INK2)
    ax.tick_params(labelsize=8.4, colors=_INK)
    for spine in ("top", "right"):
        ax.spines[spine].set_visible(False)
    for spine in ("left", "bottom"):
        ax.spines[spine].set_color(_LINE)
    grid_axis = "x" if kind == "barh" else "y"
    ax.grid(True, axis=grid_axis, color=_LINE, linewidth=0.7, zorder=0)
    ax.set_axisbelow(True)
    if multi:
        ax.legend(fontsize=8, frameon=False, labelcolor=_INK)
    if kind in ("bar", "line", "scatter") and n_max > 8:
    ...
    fig.savefig(path, format="png", facecolor=_BG, bbox_inches="tight")
    plt.close(fig)

    n_pts = sum(len(s["x"]) for s in series)
    caption = (
        f"{kind} chart, {len(series)} series, {n_pts} points"
        + (f": {title}" if title else "")
        + f". Embed it with the marker [chart:{cid}] on its own line where the chart belongs."
    )
    return f"CHART_SAVED chart_id={cid}\n{caption}"
