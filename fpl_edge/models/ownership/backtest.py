...
# --------------------------------------------------------------------------
# The fit job. Merged in from models/ownership/fit.py (ARCHITECTURE_REVIEW.md
# check 7 and Section 4 row 12): those 110 lines did nothing but drive the four
# scorers above and write the two committed JSON files, and the module they
# drove is this one.
#
# Run with `uv run python -m fpl_edge.models.ownership.backtest`.
#
# The evaluation numbers written here are leave-one-season-out and the
# parameters written here are fitted on everything. That is deliberate: the
# honest score comes from data the model never saw, and the shipped model uses
# every season available.
#
# Reads only the committed fixtures, so it is offline, deterministic and
# re-runnable.
# --------------------------------------------------------------------------
...
def main() -> None:
    ...
    ins_score, ins_by_season = loso_inseason(P)
    cold_score, cold_by_snapshot = loso_coldstart(A)
    cap_table, cap_summary = evaluate_captaincy(A)
    curve = coldstart_uncertainty_curve(A)
